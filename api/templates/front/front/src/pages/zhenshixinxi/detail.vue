	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zhenshixinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">诊室号</div>
						<div class="text "  >{{detail.zhenshihao}}</div>
					</div>
					<div class="item">
						<div class="lable">科室号</div>
						<div class="text "  >{{detail.keshihao}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">门诊类型</div>
						<div class="text "  >{{detail.menzhenleixing}}</div>
					</div>
					<div class="item">
						<div class="lable">挂号费</div>
						<div class="text "  >{{detail.guahaofei}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zhenshixinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zhenshixinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('zhenshixinxi','辅导预约')" 
							@click="onAcross('fudaoyuyue','','','','')" type="warning">
							辅导预约
						</el-button>
					</div>
				</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">医生介绍</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.yishengjieshao"></div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`zhenshixinxi`);
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('zhenshixinxi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'zhenshixinxi', userid: localStorage.getItem('frontUserid')}}).then(res => {
			...
			backClick(){
				...
				this.$router.push({path: '/index/zhenshixinxi', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			getDiscussList(page) {
				this.$http.get('discusszhenshixinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discusszhenshixinxi/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discusszhenshixinxi/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discusszhenshixinxi/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 修改
			editClick(){
				this.$router.push(`/index/zhenshixinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此诊室信息？') .then(_ => {
					this.$http.post('zhenshixinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
