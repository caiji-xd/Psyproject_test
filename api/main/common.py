...
def before_request():
    ...
    if request.method == 'GET':
        get_list = [
            "/",
            "/apidocs/",
             "/python7n2a3/examusers/login" ,
            "/python7n2a3/examusers/register",
            '/python7n2a3/defaultuser/login',
            '/python7n2a3/users/login',
        ...
        post_list = [
            '/python7n2a3/defaultuser/register',
            '/python7n2a3/defaultuser/login',
            '/python7n2a3/users/register',
            '/python7n2a3/users/login',
            "/python7n2a3/examusers/login" ,
            "/python7n2a3/examusers/register"
        ]  # 免认证list
        if request__url not in post_list and "register" not in request__url and "login" not in request__url and '/file/upload' not in request__url and '/update' not in request__url:  # 注册时不检测token。
            ...
            if result.get('code') != normal_code:
                print('jwt auth fail')
                return jsonify(result)
@main_bp.route("/python7n2a3/encrypt/md5", methods=['GET'])
def python7n2a3_encrypt_md5():
